...
CStrCastStatusCode cstr_cast_to_ssize(CString* cstr, ssize_t* ssize, int scale) {
    if (cstr == NULL || ssize == NULL || scale == 0) {
        return CStrCastFuncArgError;
    }
...
#define digit_value shift_len
            for (; cstr_id < cstr->_cstr_sz; ++cstr_id) {
                ...
                if (cstr_id == cstr->_cstr_sz - 1) {
                    if (*ssize <= SSIZE_MAX - digit_value) {
                        *ssize +=  digit_value;
                    } else {
                        return CStrCastOverflowError;
                    }
                } else {
                    if (*ssize <= (SSIZE_MAX >> 4) - digit_value) {
                        *ssize = *ssize + digit_value << 4;
                    } else {
                        return CStrCastOverflowError;
                    }
                }
            ...
            for (; cstr_id < cstr->_cstr_sz; ++cstr_id) {
                if (isdigit(cstr->_cstr[cstr_id])) {
                    ...
                    if (cstr_id == cstr->_cstr_sz - 1) {
                        ...
                        if (*ssize <= SSIZE_MAX / 10 - digit_value) {
                            *ssize = (*ssize + digit_value) * 10;
                        } else {
                            return CStrCastOverflowError;
                        }
                    }
    ...
    for (; cstr_id < cstr->_cstr_sz; ++cstr_id) {
        if (isdigit(cstr->_cstr[cstr_id])) {
            ...
            if (cstr_id == cstr->_cstr_sz - 1) {
                ...
                if (*ssize <= (SSIZE_MAX >> shift_len) - digit_value) {
                    *ssize = *ssize + digit_value << shift_len;
                } else {
                    return CStrCastOverflowError;
                }
            }
...
CStrCastStatusCode cstr_cast_to_ssize_nocheck(CString* cstr, ssize_t* ssize, int scale) {
    if (cstr->_cstr_sz == 0) {
        return CStrCastZeroStrError;
    }
...
CStrCastStatusCode cstr_ref_cast_to_ssize(CStringRef* cstr_ref, ssize_t* ssize, int scale) {
    if (cstr_ref == NULL || ssize == NULL || scale == 0) {
        return CStrCastFuncArgError;
    }
...
CStrCastStatusCode cstr_ref_cast_to_ssize_nocheck(CStringRef* cstr_ref, ssize_t* ssize, int scale) {
    if (cstr_ref->_cstr_ref_len == 0) {
        return CStrCastZeroStrError;
    }
